...
class EProfileVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tfJob.isUserInteractionEnabled = false
    ...
    func apiGetData() {
        ...
        ServerManager.shared.httpGet(request:API_BASE_URL + "user/profile/\(AppManager.getUserID())") { (response) in
            ...
            if response["status"].intValue == 200 {
                ...
                self.tfJob.text = data["services"].arrayValue.first?["name"].stringValue
...
extension EProfileVC: UITextFieldDelegate{
...
extension EProfileVC: GMSAutocompleteViewControllerDelegate {
